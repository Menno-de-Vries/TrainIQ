...
- For new or explicitly re-saved meal items, **Telt mee als vocht** plus entered grams determines per-serving hydration: checked means 1 g = 1 ml, unchecked means zero. This is a TrainIQ product rule, not density inference. Provider beverage metadata may only suggest the initial checkbox. Historical rows are never globally rewritten. Manual water logging remains a separate explicit ml flow. Each meal item retains its per-serving hydration value. A grouped Room query produces at most one linked hydration row per meal, using the meal date and serving counts. Transactional replacement/deletion therefore cannot leave a separate stale or duplicated contribution.
