...
The Android workflow is intentionally pull-request-only for automatic validation. Every pull request runs debug unit tests, lint, debug assembly, Android-test Kotlin compilation, and profileable macrobenchmark compilation with read-only repository permissions. There is no automatic `push` trigger.

`workflow_dispatch` exposes two manually triggered operations:

- `room-migration-marker` runs the connected Room migration chain on an emulator and stores the generated markers as a workflow artifact;
- `signed-release` first generates fresh Room migration markers, then checks external signing inputs and builds signed APK/AAB artifacts.

The signed-release operation uses the `trainiq-release` environment secrets documented below. It uploads build outputs only as GitHub Actions artifacts; it does not publish or deploy them. Third-party actions are pinned to immutable commit SHAs.

The `signed-release` job targets the `trainiq-release` GitHub Environment. Configure that environment with required reviewers, allowed release branches, and environment-scoped signing secrets before enabling release runs; the workflow file alone does not make manual dispatch owner-only.
...
GitHub Actions expects these `trainiq-release` environment secrets for signed release artifacts:
...
If signing inputs are absent or incomplete, release artifact tasks fail the readiness check instead of creating an unsigned or debug-signed release.
The manual `signed-release` workflow uses the same fail-closed contract and never falls back to debug signing.
...
- New Nutrition food identity allocation and immediate JSON export snapshots are resolved transactionally from Room rather than from potentially stale observed UI state.
...
- Failed Health Connect metrics discard stale change tokens, transient metric failures remain retryable, and Settings reconciles the existing background schedule after status refresh.
- Meal and scale images are bounds-decoded, sampled, resized, and JPEG-compressed off the main thread before the existing 1,500,000-byte upload boundary; invalid input stays on the local fallback path.
- Coach profile edits are owned by `CoachViewModel` and restored through `SavedStateHandle` without allowing a profile refresh to overwrite a dirty draft.
...
- Compact phones show five primary destinations: Start, Training, Voeding, Coach, and Meer. Voortgang remains a typed route and is directly available from Meer.
- Medium and expanded layouts keep all six top-level destinations in the navigation rail.
