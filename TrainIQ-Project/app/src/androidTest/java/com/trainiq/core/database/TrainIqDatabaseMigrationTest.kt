...
class TrainIqDatabaseMigrationTest {
    ...
    fun migration16To17PreservesProfileAndStartsWithNoSleepRoutine() {
        ...
        val migrated = helper.runMigrationsAndValidate(TEST_DB, 17, true)
        migrated.query("SELECT name FROM user_profile WHERE id = 1").use {
            assertTrue(it.moveToFirst())
            assertEquals("Migration Athlete", it.getString(0))
        }
        migrated.query("SELECT COUNT(*) FROM sleep_routine").use {
            assertTrue(it.moveToFirst())
