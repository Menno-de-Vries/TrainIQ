...
Last useful change: Wired Room migration-chain verification marker generation into GitHub CI with a dedicated emulator job, and made signed release artifacts depend on that gate.
...
Next safest action: Continue QA-2026-05-09-001/QA-2026-05-10-014 with one bounded persistence hot path, or run Health Connect runtime evidence if a suitable emulator/device state is available.
...
Verification summary: PASS `./gradlew.bat :app:generateCiRoomMigrationChainVerificationMarkers --dry-run --console=plain --no-configuration-cache`; PASS `./gradlew.bat :app:assembleDebug --console=plain --no-configuration-cache`. Full marker generation requires the GitHub-hosted emulator job or a connected local device.
