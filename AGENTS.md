...
- Post-merge cleanup is standing authorization for future tasks to remove their own temporary worktree and local task branch under the following checks. Record the primary worktree, temporary path, branch, remote, and PR/base when creating them; preserve the primary worktree (identify it through Git's common directory, not whichever worktree currently has `main` checked out).
- Before cleanup, fetch the base and verify the exact PR is `MERGED` into that base, its merge commit is reachable from the fetched base, and the local branch tip equals the merged PR head and is an ancestor of the base (`git merge-base --is-ancestor`). If squash/rebase prevents this proof, retain the branch for explicit review. Never clean up before merge, for open/pending/failed PRs or checks, or when status cannot be reliably established. An absent CI configuration is not a successful check.
- Recheck `git worktree list --porcelain` and `git status --porcelain=v1 --untracked-files=all` immediately before removal. Retain dirty, untracked, locked, active-in-another-task, divergent, or ambiguous worktrees; inspect ignored files and retain any user-owned evidence, secrets, or unclear data. From the primary worktree, remove only the verified temporary path with `git worktree remove` (on Windows use command-scoped `-c core.longpaths=true`), then remove its unused local task branch with `git branch -d`, then `git worktree prune`. Never force removal, delete `main`/default/protected branches, or blindly delete a folder; report partial failures and exact residual paths.
- Remote branch deletion additionally requires an existing repository convention or explicit user authorization, the same merged PR and unchanged remote head, and proof that the branch is neither protected nor used by another open PR/task. Otherwise retain it. Finish by verifying primary-worktree health, remaining paths/branches and status, and reporting removals and blockers. If the PR is still open at handoff, leave cleanup pending for the task that later verifies the merge; do not merge or introduce a background service merely to clean up.
