...
> Product routing update (2026-09-09): body measurements and their history belong to **Coach → Lichaam & voortgang**. Settings contains preferences/configuration only; historical references below to a Settings progress entry are superseded. Nutrition owns manual ml hydration and checkbox-controlled meal hydration (entered grams count 1-to-1 as ml per serving when checked). See [implementation and local verification](docs/agent-guides/coach-hydration-food-providers.md).
